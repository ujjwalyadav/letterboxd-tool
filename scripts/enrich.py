...
def film_key(row: dict[str, str]) -> str:
    ...
    if uri:
        return uri.rstrip("/")
    return f"{norm(row.get('Name'))}::{row.get('Year', '').strip()}"
...
def get_or_create(catalog: dict[str, dict[str, Any]], row: dict[str, str]) -> dict[str, Any]:
    key = film_key(row)
    if key not in catalog:
        catalog[key] = base_film(row)
    film = catalog[key]
...
def merge_letterboxd() -> dict[str, dict[str, Any]]:
    ...
    for row in read_csv("watched.csv"):
        f = get_or_create(catalog, row)
        f["user"]["watched"] = True
        f["user"]["watched_added_date"] = (row.get("Date") or "").strip() or None
    ...
    # Optional manual/authorized source for community statistics. This exists so
    # users can add Letterboxd community data without the project scraping it.
    for row in read_csv("letterboxd_community.csv"):
        key = film_key(row)
        if key in catalog:
            catalog[key]["letterboxd_community"] = {
